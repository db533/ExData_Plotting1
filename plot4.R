plot4 <- function() {
    ...
    ## Plot the 2nd chart.
    plot(x=datetime, y=data2$Voltage,type="n", 
         ylab="Voltage")
    lines(x=datetime, y=data2$Voltage)
    ...
    ## Plot the 4th chart.
    plot(x=datetime, y=data2$Global_reactive_power,type="n")
    lines(x=datetime, y=data2$Global_reactive_power, lwd=0.2)
    dev.off()
}